...
- Check CodeGraphy freshness with `codegraphy status .` or `codegraphy_repo_status` before relying on graph results for large refactors. If the index is stale, reindex in VS Code first or treat source files as the source of truth after edits.
