...
**`App.tsx`** listens for `ExtensionToWebviewMessage` events, manages all UI state, and renders the graph with panels. The toolbar includes an Export dropdown powered by `@radix-ui/react-dropdown-menu` with image exports (PNG, SVG, JPEG) and data exports (JSON, Markdown). It also hosts the Tier-2 runtime (`WebviewPluginHost`), handles `PLUGIN_WEBVIEW_INJECT`, dynamically loads plugin scripts/styles, and routes plugin-scoped messages.
