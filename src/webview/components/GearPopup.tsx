/**
 * @fileoverview Gear popup component - unified settings UI.
 * Contains view switcher and physics settings.
 * @module webview/components/GearPopup
 */

import React, { useState } from 'react';
import { IPhysicsSettings, IAvailableView } from '../../shared/types';
import { postMessage } from '../lib/vscodeApi';

interface GearPopupProps {
  physicsSettings: IPhysicsSettings;
  onPhysicsChange?: (settings: IPhysicsSettings) => void;
...
const PHYSICS_SLIDERS: SliderConfig[] = [
  {
...
/**
 * Gear popup component containing view switcher and physics settings.
 */
export function GearPopup({ 
  physicsSettings, 
  onPhysicsChange, 
  views, 
  activeViewId, 
  onViewChange 
}: GearPopupProps): React.ReactElement {
  ...
  const handleSliderChange = (key: keyof IPhysicsSettings, value: number) => {
    ...
    if (onPhysicsChange) {
      onPhysicsChange({ ...physicsSettings, [key]: value });
    }
        ...
        <div className="bg-zinc-800/95 backdrop-blur-sm rounded-lg border border-zinc-700 p-3 w-64 shadow-lg">
          ...
          {views.length > 1 && (
            <div className="mb-4 pb-4 border-b border-zinc-700">
              <label htmlFor="view-select" className="text-xs text-zinc-300 block mb-2">
                View
              </label>
              <select
                id="view-select"
                ...
                className="w-full appearance-none bg-[var(--vscode-dropdown-background,#3c3c3c)] text-[var(--vscode-dropdown-foreground,#cccccc)] border border-[var(--vscode-dropdown-border,#3c3c3c)] rounded px-3 py-1.5 text-sm cursor-pointer hover:bg-[var(--vscode-dropdown-hoverBackground,#505050)] focus:outline-none focus:ring-1 focus:ring-[var(--vscode-focusBorder,#007fd4)]"
          ...
          <div className="space-y-3">
            <span className="text-xs text-zinc-400 block">Physics</span>
            {PHYSICS_SLIDERS.map(({ key, label, min, max, step, description }) => (
              <div key={key} className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-zinc-500 font-mono">
                    {typeof physicsSettings[key] === 'number' 
                      ? physicsSettings[key].toFixed(step < 1 ? 2 : 0) 
                      : physicsSettings[key]}
                  </span>
                </div>
                <input
