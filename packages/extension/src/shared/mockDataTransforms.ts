/**
 * @fileoverview Transform functions that convert mock file data into graph nodes and edges.
 * Extracted from mockData.ts to reduce mutation sites per file.
 *
 * @module shared/mockDataTransforms
 */

import { IFileData, IGraphNode, IGraphEdge, IGraphData, getFileColor } from './types';
import { MOCK_FILE_DATA } from './mockData';
 ...
 * @returns Array of graph nodes ready for rendering
 *
 * @example
 * ```typescript
 ...
 * @returns Array of graph edges ready for rendering
 *
 * @example
 * ```typescript
 * const files: IFileData[] = [
...
/**
 * Generates complete mock graph data ready for rendering.
